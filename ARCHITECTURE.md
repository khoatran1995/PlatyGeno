...
*   **Science**: 
    1.  **Mean-Pooling**: Stabilizes discovery by averaging sequence embeddings to identify global biological "themes."
    2.  **Zero-Gate**: Removes feature bottlenecks, ensuring that rare motifs (up to 64 per read) are included.
    3.  **The Padding Filter**: When running in **Batched Mode**, sequence padding acts as a natural biological filter. This dilution effect targets the most significant "Mountain Peaks" in the genetic landscape, achieving the precise **98-feature benchmark** required for clinical discovery.
    4.  **Filtering**: Still applies `activation` strength and `rarity` checks.
